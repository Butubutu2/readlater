...
// ============================================================
// POST /api/items — 创建新收藏
// 支持客户端传入已解析的元数据以跳过重复工作
// ============================================================
export async function POST(request: NextRequest) {
  ...
  const { url, tag, title: clientTitle, cover_url: clientCoverUrl, ai_summary: clientSummary } = body as {
    url: string
    tag?: string
    title?: string
  ...
  // 3. 获取元数据（如果客户端已传入则跳过）
  let title = clientTitle || '未命名内容'
  let coverUrl: string | null = clientCoverUrl ?? null
  if (!clientTitle) {
    try {
  ...
  let aiSummary: string | null = clientSummary ?? null

  if (!tag) {
    // 只有未传 tag 且未传 ai_summary 时才需要调 AI
    if (!clientSummary) {
      const { data: tags } = await supabase
        .from('tags')
        .select('name')
      ...
      // 有 ai_summary 但没有 tag，说明客户端只提供了摘要
      const { data: tags } = await supabase
        .from('tags')
        .select('name')
        .eq('user_id', user.id)
